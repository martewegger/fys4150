...
def find_dt(solver_method):
    ...
    for i in range(len(dt_array)):
        ...
        run_func(filename, vx0=0, dt=dt_array[i], T_end=1.1, method=solver_method, compile=False)
        x, y = np.transpose(np.loadtxt(filename))
        rel_err[i] = (np.abs(np.sum(np.sqrt(x**2+y**2) - 1)))/1
...
def time_func():
    ...
    run_func(filename, vx0=0, dt=h, T_end=T, method=Euler, compile=False)
    Euler_time = np.loadtxt('timing.txt')
    run_func(filename, vx0=0, dt=h, T_end=1.1, method=Verlet, compile=False)
...
def run_and_plot():
    filename = 'outfile.txt'
    run_func(filename, vx0=0, T_end=12, method=Verlet, beta=2, compile=False)
    x,y = np.transpose(np.loadtxt(filename))
    plt.figure(figsize=(11,11))
